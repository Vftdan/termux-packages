...
termux_step_create_debscripts() {
	...
	echo "if [ ! -f $TERMUX_PREFIX/bin/pip${_MAJOR_VERSION} -a -d $TERMUX_PREFIX/lib/python${_MAJOR_VERSION}/site-packages/pip-*.dist-info ]; then rm -Rf $TERMUX_PREFIX/lib/python${_MAJOR_VERSION}/site-packages/pip-*.dist-info ; fi" >> postinst
	# Setup bin/pip:
	echo "$TERMUX_PREFIX/bin/python${_MAJOR_VERSION} -m ensurepip --upgrade --default-pip" >> postinst
	...
	echo "pip${_MAJOR_VERSION} freeze 2> /dev/null | xargs pip${_MAJOR_VERSION} uninstall -y > /dev/null 2> /dev/null" >> prerm
	...
	echo "rm -f $TERMUX_PREFIX/bin/pip${_MAJOR_VERSION} $TERMUX_PREFIX/bin/easy_install-${_MAJOR_VERSION}" >> prerm
